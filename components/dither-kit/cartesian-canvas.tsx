...
/**
 * React 19 type `RefObject<T>.current` comme non-nul ; React 18 le type
 * `T | null`. Ces refs sont toujours initialisées avant d'entrer dans la boucle
 * de rendu, d'où cet alias plutôt qu'une cinquantaine de gardes de non-nullité
 * dans du code tiers.
 */
type NonNullRef<T> = { current: T }
...
type LoopArgs = {
  ...
  cols: number
  rows: number
  state: NonNullRef<ChartContextValue>
  targets: NonNullRef<Record<string, Surface>>
  stars: NonNullRef<Star[]>
}
